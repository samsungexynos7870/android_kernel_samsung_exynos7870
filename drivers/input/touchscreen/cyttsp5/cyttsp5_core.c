		...
		struct cyttsp5_hid_desc *desc)
{
	...
	dev_info(dev, "%s: HID len:%X HID ver:%X\n", __func__,
...
#if 0 //#ifdef CONFIG_VBUS_NOTIFIER
...
//	tsp_ta_gpio = cd->pdata->core_pdata->ta_gpio;
	...
	dev_info(dev, "%s: add sysfs interfaces\n", __func__);
	...
	if (cd->cal_flag == CAL_NOT_DONE) {
		...
		dev_info(cd->dev, "%s: cal_flag is not 0, not execute calibration\n",
			__func__);
	}
	...
	rc = gpio_request(tsp_ta_gpio, "tsp_ta_gpio");
	if (rc < 0)
		pr_err("%s, tsp_ta_gpio %d request failed (%d)\n",
			__func__, tsp_ta_gpio, rc);
	else {
		rc = gpio_direction_output(tsp_ta_gpio, 0);
		if (rc)
