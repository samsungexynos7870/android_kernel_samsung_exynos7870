...
#define DEFAULT_UMS_FW_PATH		"/sdcard/synaptics.fw"
...
static void fw_update(void *dev_data)
{
	...
	if (data->cmd_param[0] == 1) {
		...
		dev_info(rmi4_data->pdev->dev.parent, "%s: user_ship, success\n", __func__);
		return;
	}
#endif
...
static void module_off_master(void *dev_data)
{
	...
	snprintf(data->cmd_buff, CMD_RESULT_STR_LEN, "OK");
	data->cmd_state = CMD_STATUS_OK;
...
static void get_chip_vendor(void *dev_data)
{
	...
	snprintf(data->cmd_buff, CMD_RESULT_STR_LEN, "SYNAPTICS");
	data->cmd_state = CMD_STATUS_OK;
	set_cmd_result(data, strlen(data->cmd_buff));
}
...
static void get_chip_name(void *dev_data)
{
	...
	snprintf(data->cmd_buff, CMD_RESULT_STR_LEN, "TD430");
	data->cmd_state = CMD_STATUS_OK;
out:
	set_cmd_result(data, strlen(data->cmd_buff));
}
...
static void run_rawcap_read(void *dev_data)
{
	...
	do {
		...
		dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to do preparation. reset and retry again.\n",
			__func__);
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to do preparation\n",
				__func__);
		snprintf(data->cmd_buff, CMD_RESULT_STR_LEN, "Error preparation");
		cmd_state = CMD_STATUS_NOT_APPLICABLE;
		goto out;
	}

	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_FULL_RAW_CAP_RX_COUPLING_COMP)) {
		snprintf(data->cmd_buff, CMD_RESULT_STR_LEN, "Error get report type");
		cmd_state = CMD_STATUS_FAIL;
		goto sw_reset;
	}
...
static void run_sidekey_delta_read(void *dev_data)
{
	...
	report_data = f54->cmd_data->absdelta_data;
	memcpy(report_data, f54->report_data, f54->report_size);
	memset(temp, 0, CMD_STR_LEN);
	memset(temp2, 0, CMD_RESULT_STR_LEN);
...
static void run_sidekey_abscap_read(void *dev_data)
{
	...
	report_data = f54->cmd_data->abscap_data;
	memcpy(report_data, f54->report_data, f54->report_size);
	memset(temp, 0, CMD_STR_LEN);
	memset(temp2, 0, CMD_RESULT_STR_LEN);
...
static void set_tsp_test_result(void *dev_data)
{
	...
	if (device_status != 0) {
		snprintf(data->cmd_buff, CMD_RESULT_STR_LEN, "NR");
		data->cmd_state = CMD_STATUS_FAIL;
...
static int synaptics_rmi4_init_ts_cmd(void)
{
	...
	data->cmd_is_running = false;
	data->ts_dev = sec_device_create(f54, "tsp");
