...
static void get_dnd_h_gap_all(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct bt532_ts_info *info = container_of(sec, struct bt532_ts_info, sec);
	struct tsp_raw_data *raw_data = info->raw_data;
	...
	for (ii = 0; ii < total_node; ii++) {
		if ((ii + 1) % (info->cap_info.y_node_num) != 0) {
			snprintf(temp, CMD_RESULT_WORD_LEN, "%d,", raw_data->hgap_data[ii]);
...
static void get_dnd_v_gap_all(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct bt532_ts_info *info = container_of(sec, struct bt532_ts_info, sec);
	struct tsp_raw_data *raw_data = info->raw_data;
	...
	char *buff = NULL;
	int ii;
	int total_vgap_node = (info->cap_info.x_node_num - 1) * info->cap_info.y_node_num;

	sec_cmd_set_default_result(sec);

	buff = kzalloc(total_vgap_node * CMD_RESULT_WORD_LEN, GFP_KERNEL);
	if (!buff)
		return;

	for (ii = 0; ii < total_vgap_node; ii++) {
		snprintf(temp, CMD_RESULT_WORD_LEN, "%d,", raw_data->vgap_data[ii]);
		strncat(buff, temp, CMD_RESULT_WORD_LEN);
		memset(temp, 0x00, SEC_CMD_STR_LEN);
	}

	sec_cmd_set_cmd_result(sec, buff, strnlen(buff, total_vgap_node * CMD_RESULT_WORD_LEN));
	sec->cmd_state = SEC_CMD_STATUS_OK;
	kfree(buff);
}
...
static struct sec_cmd sec_cmds[] = {
	...
	{SEC_CMD("get_dnd_v_gap_all", get_dnd_v_gap_all),},
	{SEC_CMD("run_dnd_h_gap_read", run_dnd_h_gap_read),},
	{SEC_CMD("get_dnd_h_gap", get_dnd_h_gap),},
	{SEC_CMD("get_dnd_h_gap_all", get_dnd_h_gap_all),},
